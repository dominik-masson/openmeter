...
class _ContractCardState extends State<ContractCard> {
  Future<bool> _deleteContract( BuildContext context,  int contractId,  int providerId) async {
    return await showDialog(
          context: context,
          builder: (context) => AlertDialog(
            title: const Text('Sind Sie sich sicher?'),
            content:
                const Text('Möchten Sie diesen Vertrag wirklich löschen?'),
            actions: [
              TextButton(
              ...
              TextButton(
                onPressed: () {
                  if(providerId != -1){
                    Provider.of<LocalDatabase>(context, listen: false).contractDao.deleteProvider(providerId );
                  }
                  Provider.of<LocalDatabase>(context, listen: false).contractDao.deleteContract(contractId);
            ...
            return Padding(
              ...
              child: Dismissible(
                ...
                confirmDismiss: (direction) async {
                  return await _deleteContract(context, contract.uid, contract.provider);
                },
